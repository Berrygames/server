...
export default async function berryAddCommand(url: string, message: any) {
      ...
      const location = message.content.split(" ")[2];
      ...
      if (!mentionedUser) {
            message.reply(
                  buildEmbed({
                        ...
                        description:
                              "Please mention a user to give a berry to.",
                        color: "#FF0000",
                  }),
            );
            return;
      ...
      if (location !== "cash" && location !== "bank") {
            message.reply(
                  buildEmbed({
                        author: message.author,
                        description:
                              "Please specify a valid location.\nValid locations are `cash` and `bank`.",
                        color: "#FF0000",
                  }),
            );
            return;
      ...
      try {
            ...
            message.reply(
                  buildEmbed({
                        author: message.author,
                        description:
                              "An error occurred while trying to give a berry.",
                        color: "#FF0000",
                  }),
            );
      }
