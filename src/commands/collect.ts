      ...
      try {
            ...
            if (succesfulRoles.length === 0) {
                  ...
                  const cooldownText = failedRoles
                        .map(
                              (r, index) =>
                                    `${index + 1}. <@&${r.role.roleId}> on cooldown until ${r.result.nextCollectionTimestamp}`,
                        )
                        .join("\n");
            ...
            const successText = succesfulRoles
                  .map((r, index) => {
                        return `${index + 1}. Collected ${emojiString}${r.role.amount} from <@&${r.role.roleId}>`;
                  })
                  .join("\n");
            ...
            if (failedRoles.length > 0) {
                  const cooldownText = failedRoles
                        .map(
                              (r) =>
                                    `${succesfulRoles.length + 1}. <@&${r.role.roleId}> on cooldown until ${r.result.nextCollectionTimestamp}`,
                        )
                        .join("\n");
                  description += `\n${cooldownText}`;
            }
