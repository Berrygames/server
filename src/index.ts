...
export const activeGames = new Map<string, (interaction: any) => void>();
...
client.on("messageCreate", async (message) => {
    ...
    */
      ...
      // blackjack
            ...
            commands["blackjack"].commands.some((cmd) =>
                  message.content.startsWith(cmd),
            )
      ) {
            blackjack(API_URL, message, client);
      }
...
client.on("interactionCreate", async (interaction) => {
      if (interaction.isButton()) {
            const handler = activeGames.get(interaction.message.id);
            if (handler) await handler(interaction);
            return;
      }
      if (interaction.type !== InteractionType.ApplicationCommand) return;
      const handler = activeGames.get(interaction.commandName);
